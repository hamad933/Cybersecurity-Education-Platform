...
<template>
  <section aria-labelledby="source-comparison-heading">
    <div class="flex flex-wrap items-end justify-between gap-3 border-b border-slate-800/80 pb-3">
      <div>
        <span class="font-mono text-[10px] font-bold text-slate-500 uppercase tracking-widest" dir="ltr">COMPARE</span>
        <h2 id="source-comparison-heading" class="mt-1 text-base font-bold text-slate-100">مقارنة المصادر</h2>
      </div>
      <p class="text-xs text-slate-400">مقارنة وصفية، وليست ترتيبًا للحقيقة.</p>
    </div>

    <div v-if="rows.length" class="mt-4 overflow-x-auto rounded-2xl border border-slate-800/80 bg-slate-950/60 shadow-lg">
      <table class="min-w-full divide-y divide-slate-800/80 text-xs">
        ...
        <tbody class="divide-y divide-slate-800/60">
          <tr v-for="row in rows" :key="row.source_id" class="hover:bg-slate-900/40 transition">
            <td class="px-4 py-3">
              <p class="font-bold text-slate-200 text-sm">{{ row.title }}</p>
              <bdi dir="ltr" class="mt-0.5 block font-mono text-[10px] text-slate-500">
                {{ row.source_id }}
              </bdi>
            </td>
            <td class="px-4 py-3">
              <span class="rounded-full bg-slate-900 border border-slate-800 px-2 py-0.5 font-mono text-[10px] text-slate-300">
                {{ row.authority_class }}
              </span>
            </td>
            <td class="px-4 py-3 font-mono font-semibold text-slate-300">{{ row.claim_count }}</td>
            <td class="px-4 py-3 font-mono font-bold text-cyan-300">{{ row.active_revision_claim_count }}</td>
            ...
            <td class="px-4 py-3">
              <span
                ...
                :class="row.has_integrity_digest ? 'bg-emerald-950/80 border border-emerald-500/40 text-emerald-300' : 'bg-amber-950/80 border border-amber-500/40 text-amber-300'"
              >
