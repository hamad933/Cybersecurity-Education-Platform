...
type Node = { id: string; kind: 'knowledge_unit' | 'capability'; label: string; technical_label: string };
type Edge = { id: string; from: string; to: string; type: string; revision: number; lifecycle: Record<string, unknown> };
...
const unitNode = computed(() => props.graph.nodes.find((node) => node.kind === 'knowledge_unit') ?? null);
</script>
      ...
      <div class="mt-4 grid min-h-[700px] gap-4 xl:grid-cols-[260px_minmax(0,1fr)_280px]">
        <aside class="rounded-xl border border-slate-800 bg-slate-900/50 p-4" aria-label="الخريطة والنطاق">
          ...
          <div class="mt-3 rounded-lg border border-dashed border-slate-700 p-3">
            ...
            <bdi dir="ltr" class="mt-2 block font-mono text-[10px] text-slate-600">{{ map.state }}</bdi>
          </div>
        ...
        <main class="min-w-0 rounded-xl border border-slate-800 bg-slate-900/35 p-5 sm:p-7">
          ...
          <section v-if="active && graph.nodes.length" class="mt-8">
            ...
            <div v-else class="grid gap-4 md:grid-cols-[1fr_auto_1fr] md:items-center">
              <div class="space-y-3">
                <article v-for="capability in capabilities" :key="capability.id" class="rounded-xl border border-indigo-800 bg-indigo-950/20 p-4 text-center">
                  <bdi dir="ltr" class="font-mono text-sm text-indigo-200">{{ capability.technical_label }}</bdi>
                </article>
                <p v-if="!capabilities.length" class="text-center text-sm text-slate-500">لا توجد عقد Capability مرتبطة.</p>
              </div>
              <div class="px-4 text-center text-slate-600" aria-hidden="true">⟷</div>
              <article v-if="unitNode" class="rounded-xl border border-cyan-700 bg-cyan-950/20 p-5 text-center">
                <p class="font-bold">{{ unitNode.label }}</p>
                <bdi dir="ltr" class="mt-1 block font-mono text-xs text-cyan-200">{{ unitNode.technical_label }}</bdi>
              </article>
            </div>
          ...
          <div v-else class="grid min-h-[420px] place-items-center text-center">
            <div><h1 class="font-bold text-slate-300">لا توجد علاقات قابلة للتصوّر.</h1><p class="mt-2 text-sm text-slate-500">المشهد لا يختلق Nodes أو Edges.</p></div>
          </div>
        </main>
        ...
        <aside class="rounded-xl border border-slate-800 bg-slate-900/50 p-4" aria-label="سياق التصوّر">
          ...
          <section class="mt-6 border-t border-slate-800 pt-5">
            ...
            <bdi dir="ltr" class="mt-2 block font-mono text-xs text-slate-300">{{ graph.source }}</bdi>
            <p class="mt-3 text-xs leading-6 text-slate-500">تغيير View يغيّر التمثيل فقط. لا توجد عملية كتابة تغيّر canonical containment.</p>
          </section>

          <section class="mt-6 border-t border-slate-800 pt-5 text-sm">
            <p>العقد: <bdi dir="ltr" class="font-mono">{{ graph.nodes.length }}</bdi></p>
            <p class="mt-2">العلاقات: <bdi dir="ltr" class="font-mono">{{ graph.edges.length }}</bdi></p>
          </section>
        </aside>
      </div>
    </div>
