...
const handoffClaim = ref('مرشح دليل مشتق من نتيجة المحاكاة المختومة؛ يخضع لاحقًا لعملية Intake في Progress & Evidence.');
    ...
    <div class="workspace" dir="rtl">
        <header class="topbar">
            <div>
                ...
                <h1>{{ pageTitle }}</h1>
                <p class="subtitle">تنفيذ داخلي عالي الدقة قائم على الحالة والسببية والأحداث والـ Telemetry والتحقق.</p>
            </div>
            <div v-if="section === 'scenarios' || section === 'labs'" class="prepare-tools" aria-label="أدوات التهيئة">
            ...
            <div v-else-if="section === 'runs' && selectedRun" class="run-actions" aria-label="إجراءات التشغيل الحالية">
                <button v-if="selectedRun.available_actions.includes('ready')" type="button" @click="runAction(selectedRun, 'ready')">اعتماد الجاهزية</button>
                <button v-if="selectedRun.available_actions.includes('start')" type="button" @click="runAction(selectedRun, 'start')">بدء</button>
                <button v-if="selectedRun.available_actions.includes('pause')" type="button" @click="runAction(selectedRun, 'pause')">إيقاف مؤقت</button>
                <button v-if="selectedRun.available_actions.includes('resume')" type="button" @click="runAction(selectedRun, 'resume')">استئناف</button>
                <button v-if="selectedRun.available_actions.includes('complete')" type="button" @click="runAction(selectedRun, 'complete')">إكمال المحاكاة الداخلية</button>
                <button v-if="selectedRun.available_actions.includes('snapshot')" type="button" @click="runAction(selectedRun, 'snapshot')">حفظ Snapshot</button>
                <button v-if="selectedRun.available_actions.includes('stop')" class="button-muted" type="button" @click="runAction(selectedRun, 'stop')">إيقاف</button>
            </div>
        </header>
        ...
        <div class="body-grid">
            ...
            <main class="center-surface">
                <div v-if="records.length === 0" class="empty-state">
                    ...
                    <p>هذه الواجهة تقرأ الحالة الفعلية من PostgreSQL. لم تُنشأ تعريفات منشورة في هذا النطاق بعد.</p>
                </div>
                ...
                <template v-if="section === 'enterprise'">
                    <article v-for="enterprise in enterprises" :key="enterprise.id" class="record-card" :class="{ selected: selectedId === enterprise.id }" @click="selectedId = enterprise.id">
                        <div class="record-heading">
                            <div><span v-if="enterprise.is_fixture" class="fixture-badge">بيانات تجريبية</span><h2>{{ enterprise.name_ar }}</h2></div>
                            <span class="technical slug">{{ enterprise.slug }}</span>
                        </div>
                        <div class="causal-strip">
                            <span>Enterprise</span><b>←</b><span>Digital Twin Revision {{ enterprise.digital_twin_revision?.revision ?? '—' }}</span><b>←</b><span>Baseline {{ enterprise.baseline?.revision ?? '—' }}</span>
                        </div>
                    ...
                    <article v-for="scenario in scenarios" :key="scenario.id" class="record-card" :class="{ selected: selectedId === scenario.id }" @click="selectedId = scenario.id">
                        ...
                        <div v-if="scenario.lab_module_references.length" class="module-list">
                            <div v-for="module in scenario.lab_module_references" :key="module.reference_id" class="module-row">
                                <span>{{ module.lab_title_ar }}</span><code>{{ module.module_key }}</code><small>Reference → Lab Definition</small>
                            </div>
                        </div>
                ...
                <template v-else-if="section === 'results'">
                    <article v-for="result in results" :key="result.id" class="record-card result-card" :class="{ selected: selectedId === result.id }" @click="selectedId = result.id">
                        <div class="record-heading">
                            <div><h2>نتيجة تشغيل مختومة</h2><p class="technical id-line">Run {{ result.run_id }}</p></div>
                        ...
                        <div class="runtime-facts"><span>Run Lifecycle <b class="technical">{{ result.run_lifecycle }}</b></span><span>Score <b class="technical">{{ result.score ?? '—' }}</b></span><span>Sealed <b class="technical">{{ result.sealed_at }}</b></span></div>
            ...
            <aside class="right-context" aria-label="السياق الفريد">
                <template v-if="selectedEnterprise">
                    <h2>سياق المؤسسة</h2>
                    <p>{{ selectedEnterprise.description_ar || 'تعريف محاكاة مؤسسية داخلية مثبتة وقابلة لإعادة التنفيذ.' }}</p>
                    <h3>خصائص التعريف</h3><pre class="technical">{{ jsonText(selectedEnterprise.definition) }}</pre>
                    <h3>سلامة النسخ</h3><p>Digital Twin وBaseline مثبتان بواسطة digests، ولا يتم اشتقاقهما من حالة Run المتغيرة.</p>
                </template>
                <template v-else-if="selectedScenario">
                    ...
                    <p class="context-rule">Scenario هو كائن orchestration مستقل؛ مرجع Lab لا يتحول إلى Standalone Lab Run داخل Scenario Run.</p>
                </template>
                <template v-else-if="selectedLab">
                    <h2>سياق المختبر</h2>
                    <h3>Validation</h3><pre class="technical">{{ jsonText(selectedLab.validation) }}</pre>
                    <h3>مرجع Baseline</h3><code class="technical wrap">{{ selectedLab.baseline_id }}</code>
                ...
                <template v-else-if="selectedRun">
                    ...
                    <form v-if="['COMPLETED','STOPPED','FAILED'].includes(selectedRun.lifecycle) && !selectedRun.result_id" class="action-form" @submit.prevent="sealSelectedRun">
                        ...
                        <label>التفسير<textarea v-model="resultSummary" rows="4" /></label>
                    ...
                    <p v-else-if="selectedRun.result_id" class="sealed-note">لهذا التشغيل Result مختوم بالفعل؛ لا يمكن إعادة كتابة التاريخ.</p>
                </template>
                <template v-else-if="selectedResult">
                    ...
                    <div v-if="selectedResult.candidate_evidence_handoff" class="handoff-state">
                        ...
                        <p>تم إنشاء Handoff فقط. قبول Evidence والمراجعة وMastery ليست ملكًا لـ Simulation &amp; Enterprise.</p>
                    </div>
