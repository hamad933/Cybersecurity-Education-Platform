...
const primaryNavigation = computed(() =>
  PRIMARY_KEYS.map((key) =>
    props.navigation.find((item) => item.key === key),
...
function runAction(action: string): void {
  if (selectedRun.value)
    post(`/simulation/runs/${selectedRun.value.id}/${action}`);
}
function sealRun(payload: {
  ...
  if (selectedRun.value)
    post(`/simulation/runs/${selectedRun.value.id}/result`, payload);
}
  ...
  <div class="simulation-workspace" dir="rtl">
    <header class="top-zone" data-zone="top">
      <div>
        ...
        <div class="title-row">
          <h1>{{ pageTitle }}</h1>
          <span class="scope-badge">Internal High-Fidelity Simulation</span>
        </div>
        <p class="subtitle">
          سطح عمل تشغيلي يميّز التعريفات المثبتة، وحالة التشغيل، والحقائق
          التاريخية المختومة.
        </p>
      </div>
      <div
      ...
      >
        <label
          ...
          ><input
            v-model.number="seed"
            class="technical compact-input"
            type="number"
            min="0"
        /></label>
        <label
          ><span>Mode</span
          ><select v-model="mode" class="technical">
      ...
      <div v-else-if="selectedRun" class="top-context">
        <span>الحالة الحالية</span
        ><strong class="technical state-chip" dir="ltr">{{
          selectedRun.lifecycle
        }}</strong>
      </div>
      <div v-else-if="selectedResult" class="top-context">
        <span>تاريخ مختوم</span
        ><strong class="technical state-chip" dir="ltr">{{
    ...
    <div class="body-grid">
      <aside
        class="left-zone"
        data-zone="left"
        aria-label="التنقل وبنية النطاق"
      >
        <p class="rail-kicker">المناطق الرئيسية</p>
        <nav
          class="primary-nav"
          data-testid="simulation-primary-nav"
          aria-label="المناطق الرئيسية"
        >
          <Link
            ...
            ><small class="technical" dir="ltr">{{ item.key }}</small></Link
          >
        </nav>
        <section class="catalog-section">
          <p class="rail-kicker">الفهرس الحالي</p>
          <h2>{{ pageTitle }}</h2>
          <div
            v-if="records.length"
            class="record-list"
          ...
          <p v-else class="rail-empty">
            لا توجد سجلات مستلمة من الخادم في هذا النطاق.
          </p>
        </section>
      </aside>

      <main class="center-zone" data-zone="center">
        <section v-if="!records.length" class="empty-state">
          <span>—</span>
          <h2>لا توجد حالة تشغيلية متاحة</h2>
          <p>
            تعرض هذه الصفحة بيانات الخادم الفعلية فقط، ولن تنشئ تعريفات أو
            قياسات بديلة.
          </p>
        </section>
        <EnterpriseCanvas
          v-else-if="selectedEnterprise"
