...
const emit = defineEmits<{
  ...
  seal: [
    payload: { outcome: string; summary_ar: string; score: number | null },
  ];
}>();
...
const summary = ref(
  "لم يُطبّق تقييم نهائي بعد؛ تم ختم الحقائق التشغيلية كما هي.",
);
const score = ref<number | null>(null);
const runtimeFields = computed(() =>
  fieldEntries(props.run.runtime_state, ["telemetry", "validation"]),
);
const telemetry = computed(() =>
  fieldEntries(props.run.runtime_state.telemetry),
);
const validation = computed(() =>
  fieldEntries(props.run.runtime_state.validation),
);
  ...
  <section class="surface-panel" data-testid="run-runtime-console">
    ...
    <template v-if="mode === 'runtime'">
      <div class="machine-facts">
        ...
        <article>
          <small>الحالة</small
          ><strong class="technical" dir="ltr">{{ run.lifecycle }}</strong>
        </article>
        <article>
          <small>Seed</small
          ><strong class="technical" dir="ltr">{{ run.seed }}</strong>
        </article>
        <article>
          <small>Input Digest</small
          ><strong class="technical digest" dir="ltr">{{
            run.input_digest
          }}</strong>
        </article>
      </div>
      ...
      <section class="subsurface">
        ...
        <p v-else class="truthful-unavailable">
          لا توجد حقول Runtime State إضافية مستلمة.
        </p>
      </section>
      ...
      <div class="runtime-columns">
        <article>
          ...
          <p v-else class="truthful-unavailable">
            لا توجد Telemetry منظّمة مستلمة.
          </p>
        </article>
        <article>
          ...
          <p v-else class="truthful-unavailable">
            لا توجد Validation منفصلة مستلمة.
          </p>
        </article>
      </div>
    </template>
    ...
    <section v-else class="operations-mode" data-testid="run-operations-panel">
      <div class="operations-header">
        <div>
          ...
          <h3>إجراءات الحالة المصرّح بها لهذا التشغيل</h3>
        </div>
        <span class="technical state-chip" dir="ltr">{{ run.lifecycle }}</span>
      </div>
      <div v-if="run.available_actions.length" class="operation-actions">
        <button
          v-if="run.available_actions.includes('ready')"
          type="button"
          @click="emit('action', 'ready')"
        >
          اعتماد الجاهزية
        </button>
        <button
          v-if="run.available_actions.includes('start')"
          type="button"
        ...
        <button
          v-if="run.available_actions.includes('pause')"
          type="button"
        ...
        <button
          v-if="run.available_actions.includes('resume')"
          type="button"
        ...
        <button
          v-if="run.available_actions.includes('complete')"
          type="button"
        ...
        <button
          v-if="run.available_actions.includes('snapshot')"
          type="button"
          @click="emit('action', 'snapshot')"
        >
          حفظ Snapshot
        </button>
        <button
          v-if="run.available_actions.includes('stop')"
          class="button-muted"
          type="button"
      ...
      <p v-else class="truthful-unavailable">
        لا يعلن الخادم إجراءات انتقال متاحة لهذه الحالة.
      </p>

      <form
        v-if="
          ['COMPLETED', 'STOPPED', 'FAILED'].includes(run.lifecycle) &&
          !run.result_id
        "
        class="seal-form"
        @submit.prevent="seal"
      >
        <p class="rail-kicker">Seal Result</p>
        <h3>ختم النتيجة التاريخية</h3>
        <label
          ><span>Outcome</span
          ><select v-model="outcome" class="technical">
            <option v-for="item in outcomes" :key="item">{{ item }}</option>
          </select></label
        >
        <label
          ><span>التفسير المسجل</span
          ><textarea v-model="summary" rows="4" maxlength="2000" />
        </label>
        <label
          ><span>Score — اختياري</span
          ><input
      ...
      <p v-else-if="run.result_id" class="sealed-note">
        يوجد Result مختوم لهذا التشغيل؛ لا تعرض Operations إجراءً لإعادة كتابة
        التاريخ.
      </p>
    </section>
  </section>
