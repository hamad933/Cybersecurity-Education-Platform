...
import type { LabItem, OrderedDefinitionItem } from '../types';
...
function getStepTheme(index: number): {
  ...
  const mapping: Record<number, { icon: string; theme: 'green' | 'blue' | 'purple' | 'orange' | 'teal'; subtitle: string }> = {
    ...
    2: { icon: 'target', theme: 'purple', subtitle: 'Determine if input manipulation is accepted.' },
    3: { icon: 'database', theme: 'orange', subtitle: 'Verify expected changes in simulated data.' },
    4: { icon: 'activity', theme: 'teal', subtitle: 'Analyze logs and outputs to explain behavior.' },
  };
  return mapping[index] ?? { icon: 'check-square', theme: 'blue', subtitle: 'Execute step instructions.' };
}
</script>
        ...
        <div v-if="steps.length" class="sim-task-graph" role="list" aria-label="مسار مهام المختبر">
          <template v-for="(step, index) in steps" :key="step.id">
            ...
            >
              <div class="sim-task-node__inner">
                <div class="sim-task-node__icon">
                  <svg v-if="getStepTheme(index).icon === 'globe'" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>
                  <svg v-else-if="getStepTheme(index).icon === 'search'" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                  <svg v-else-if="getStepTheme(index).icon === 'target'" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>
                  <svg v-else-if="getStepTheme(index).icon === 'database'" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3"/><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"/></svg>
                  <svg v-else width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>
                </div>
            ...
            <div v-if="index < steps.length - 1" class="sim-task-connector" aria-hidden="true">
              <svg width="32" height="24" viewBox="0 0 32 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="2" y1="12" x2="28" y2="12"/><polyline points="20 5 28 12 20 19"/></svg>
            </div>
          </template>
        </div>
