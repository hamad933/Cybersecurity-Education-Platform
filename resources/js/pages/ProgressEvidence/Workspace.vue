  ...
  <main class="workspace" dir="rtl">
    <header class="top-bar">
      ...
      <div class="top-actions" aria-label="إجراءات سير العمل الحالي">
        <template v-if="surface === 'evidence'">
          <button class="button secondary" type="button" @click="panel = 'intake'">Candidate جديد</button>
        ...
        <button v-else-if="surface === 'mastery'" class="button primary" type="button" @click="openMastery">
          إلحاق Mastery State
        </button>
    ...
    <section v-if="panel" class="bottom-workspace panel" aria-label="مساحة العمل المؤقتة">
      ...
      <form v-if="panel === 'intake'" class="form-grid" @submit.prevent="intake.post('/progress/intake', { onSuccess: () => (panel = null) })">
        <label>نوع المصدر · Source Type<input v-model="intake.source_type" dir="ltr" required /></label>
        <label>معرّف المصدر · Source ID<input v-model="intake.source_id" dir="ltr" required /></label>
        <label>مراجعة المصدر · Source Revision<input v-model="intake.source_revision" dir="ltr" required /></label>
        <label>بصمة المصدر · SHA-256<input v-model="intake.source_digest" dir="ltr" minlength="64" maxlength="64" required /></label>
        <label>المادة المختارة<input v-model="intake.selected_material_refs[0]" dir="ltr" required /></label>
      ...
      <form v-else-if="panel === 'mastery'" class="form-grid" @submit.prevent="masteryForm.post('/progress/mastery/evaluate', { onSuccess: () => (panel = null) })">
        ...
        <label>Policy Revision<input v-model="masteryForm.policy_revision_id" dir="ltr" required /></label>
        <label>Judgment<select v-model="masteryForm.judgment" dir="ltr"><option>NOT_EVALUATED</option><option>INSUFFICIENT_EVIDENCE</option><option>INCONCLUSIVE</option><option>NOT_MASTERED</option><option>MASTERED</option></select></label>
