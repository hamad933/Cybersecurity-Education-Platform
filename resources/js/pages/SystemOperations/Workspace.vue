  ...
  <div class="system-workspace" dir="rtl">
    <header class="workspace-top">
      ...
      <div class="top-actions" aria-label="أدوات المساحة الحالية">
        ...
        <a v-if="surface === 'ai-bridge'" href="#manual-ai-export" class="tool-link">تجهيز Prompt</a>
    ...
    <div class="workspace-grid">
      ...
      <main class="workspace-center">
              ...
              <article v-for="run in state.processing?.runs ?? []" :key="run.id" class="trace-card">
                ...
                <dl class="trace-grid">
                  ...
                  <div>
                    ...
                    <dd>{{ when(run.started_at) }} → {{ when(run.completed_at || run.cancelled_at) }}</dd>
                  </div>
                </dl>
                <p v-if="run.safe_error_message" class="diagnostic">
                  <bdi dir="ltr">{{ run.error_category || 'uncategorized' }}</bdi> · {{ run.safe_error_message }}
                </p>
              </article>
            ...
            <div class="record-list">
              <article v-for="pkg in packagesAsRecords" :key="pkg.id" class="trace-card">
                ...
                <details class="context-disclosure">
                  ...
                  <h3>Scope</h3><pre dir="ltr">{{ jsonText(pkg.scope) }}</pre>
                  <h3>Manifest</h3><pre dir="ltr">{{ jsonText(pkg.manifest) }}</pre>
                </details>
              </article>
        ...
        <template v-else-if="surface === 'ai-bridge'">
          <section class="panel" id="manual-ai-export">
            ...
            <form
              class="form-grid"
              @submit.prevent="promptForm.post('/system/ai-bridge/prompts/export', { preserveScroll: true })"
            >
          ...
          <section class="panel">
            ...
            <form
              class="inline-form"
              @submit.prevent="aiImportForm.post('/system/ai-bridge/results/import', { forceFormData: true, preserveScroll: true })"
            >
            ...
            <div class="record-list">
              <article v-for="revision in state.prompt_revisions ?? []" :key="revision.id" class="trace-card">
                ...
                <dl class="trace-grid">
                  ...
                  <div class="wide"><dt>Package digest</dt><dd class="mono break-all" dir="ltr">{{ revision.package_digest }}</dd></div>
                </dl>
                <details class="context-disclosure">
                  <summary>Declared scope، target، method وpackage context</summary>
                  <h3>Declared scope</h3><pre dir="ltr">{{ jsonText(revision.declared_scope) }}</pre>
                  <h3>Package scope</h3><pre dir="ltr">{{ jsonText(revision.package_scope) }}</pre>
                  <h3>Package manifest</h3><pre dir="ltr">{{ jsonText(revision.package_manifest) }}</pre>
                </details>
              </article>
          ...
          <section class="panel">
            ...
            <p class="muted">لا تُعرض معاينة مختصرة لاتخاذ القرار. افتح سجل النتيجة لمراجعة proposal الكامل وProvenance ثم تظهر أدوات القرار.</p>
            <div class="record-list">
              <article v-for="result in state.results ?? []" :key="result.id" class="trace-card result-card">
                ...
                <details class="proposal-review">
                  ...
                  <div class="review-section">
                    ...
                    <pre class="proposal-payload" dir="ltr">{{ jsonText(result.structured_result) }}</pre>
                  </div>
                  <div class="review-section">
                  ...
                  <dl class="trace-grid">
                    ...
                    <div><dt>Prompt package identity</dt><dd class="mono" dir="ltr">{{ result.prompt_portable_package_id }}</dd></div>
                    <div><dt>Returned package type</dt><dd dir="ltr">{{ result.returned_package_type }}</dd></div>
                    <div><dt>Returned package state</dt><dd dir="ltr">{{ result.returned_package_status }}</dd></div>
          ...
          <section class="panel" v-if="(state.decisions ?? []).length > 0">
        ...
        <template v-else-if="surface === 'backups'">
          ...
          <details class="danger-zone">
            ...
            <div class="danger-body">
              ...
              <form
                class="inline-form"
                @submit.prevent="restoreForm.post('/system/backups/restores/stage', { forceFormData: true, preserveScroll: true })"
              >
          ...
          <section class="panel">
            ...
            <div class="record-list">
              <article v-for="restore in state.restores ?? []" :key="restore.id" class="trace-card">
                ...
                <details class="context-disclosure"><summary>Verification المسجل</summary><pre dir="ltr">{{ jsonText(restore.verification) }}</pre></details>
              </article>
          ...
          <section class="panel">
            ...
            <div class="record-list">
              <article v-for="record in state.records ?? []" :key="record.id" class="trace-card audit-card">
                <div class="trace-heading">
                  <div>
                    <strong><bdi dir="ltr">#{{ record.sequence_no }} · {{ record.action }}</bdi></strong>
                ...
                <dl class="trace-grid">
                  ...
                  <div><dt>Target type</dt><dd class="mono" dir="ltr">{{ record.target_type }}</dd></div>
                  <div><dt>Target identifier</dt><dd class="mono" dir="ltr">{{ record.target_identifier || '—' }}</dd></div>
                  <div><dt>Correlation ID</dt><dd class="mono" dir="ltr">{{ record.correlation_id }}</dd></div>
                </dl>
                <details class="context-disclosure">
                  <summary>Safe metadata وHash chain context</summary>
                  <h3>Safe metadata</h3><pre dir="ltr">{{ jsonText(record.safe_metadata) }}</pre>
                  <dl class="trace-grid">
                    <div class="wide"><dt>Previous hash</dt><dd class="mono break-all" dir="ltr">{{ record.previous_hash || 'GENESIS' }}</dd></div>
                    <div class="wide"><dt>Record hash</dt><dd class="mono break-all" dir="ltr">{{ record.record_hash }}</dd></div>
                  </dl>
                </details>
              </article>
        ...
        <template v-else-if="surface === 'releases'">
          ...
          <section class="panel">
            ...
            <p class="muted">هذه بيانات الحزمة المسجلة نفسها: الهوية، النطاق، Manifest والحالة. لا تمنح صلاحية Deployment.</p>
            <div class="record-list">
              <article v-for="pkg in packagesAsRecords" :key="pkg.id" class="trace-card">
                <div class="trace-heading">
      ...
      <aside class="workspace-right" aria-label="السياق الفريد">
        ...
        <template v-else-if="surface === 'validation'">
          ...
          <p>التحقق هنا يختبر البنية، النوع، provenance، hashes وحدود الحزمة. قرار جودة المعرفة يبقى خارج W05.</p>
        </template>
        <template v-else-if="surface === 'ai-bridge'">
        ...
        <template v-else-if="surface === 'audit'">
          ...
          <p>Actor وTarget وCorrelation وSafe metadata والـhashes معروضة للتحقيق؛ لا توجد أوامر تعديل للسجل.</p>
        </template>
        <template v-else-if="surface === 'releases'">
        ...
        <template v-else-if="surface === 'configuration'">
          <p class="rail-label">نطاق التهيئة</p><h2>تشغيل المنتج المحلي</h2>
...
.workspace-top h1 { margin: 0; font-size: 28px; }
.subtitle { margin: 8px 0 0; color: #9fb0bd; max-width: 760px; line-height: 1.8; }
...
.read-only-chip { cursor: default; border-color: #33434f; background: #101c25; color: #9fb0bd; }
.workspace-grid { display: grid; grid-template-columns: 200px minmax(0, 1fr) 260px; gap: 16px; }
.workspace-left,
.workspace-right,
.workspace-center { border: 1px solid #22313c; background: #09141c; border-radius: 16px; }
.workspace-left { padding: 16px; align-self: start; position: sticky; top: 16px; }
.rail-link { display: block; color: #aebbc5; text-decoration: none; padding: 11px 12px; border-radius: 9px; margin: 3px 0; }
.rail-link.active { background: #12343a; color: #e8ffff; border-inline-start: 3px solid #67d1cf; }
.workspace-center { padding: 18px; min-width: 0; }
.workspace-right { padding: 18px; align-self: start; position: sticky; top: 16px; color: #b5c1ca; line-height: 1.8; }
.workspace-right h2 { color: #eef7fa; margin: 4px 0 12px; }
.panel,
.hero-state,
.danger-zone { border: 1px solid #263741; background: #0d1922; border-radius: 14px; padding: 18px; margin-bottom: 14px; }
.hero-state { display: flex; justify-content: space-between; gap: 16px; align-items: center; }
.hero-state h2,
.panel h2 { margin: 0 0 14px; font-size: 20px; }
.status-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 10px; }
.status-card { display: flex; justify-content: space-between; gap: 12px; align-items: center; border: 1px solid #243945; background: #0a151d; border-radius: 11px; padding: 13px; }
.status-card.wide { align-items: flex-start; }
.status-card p { margin: 6px 0 0; color: #96a8b5; font-size: 13px; line-height: 1.6; }
.state-pill { display: inline-flex; align-items: center; white-space: nowrap; border: 1px solid #3b505e; background: #111f28; border-radius: 999px; padding: 4px 9px; font-size: 11px; font-weight: 900; color: #cbd8df; }
.state-pill.ok { border-color: #2e6e61; color: #9be3ce; background: #0d2924; }
.state-pill.danger { border-color: #7d4347; color: #ffc0c1; background: #2a1518; }
.metric-strip { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 10px; }
.metric-strip.compact { grid-template-columns: repeat(4, minmax(0, 1fr)); margin-bottom: 16px; }
.metric-strip div,
.trace-grid div,
.config-list div { border: 1px solid #253842; background: #0a151d; border-radius: 10px; padding: 12px; }
.metric-strip span { display: block; color: #8fa2af; font-size: 12px; }
.metric-strip strong { display: block; font-size: 24px; margin-top: 5px; }
.record-list { display: grid; gap: 9px; }
.record-row,
.trace-card { border: 1px solid #243741; background: #0a151d; border-radius: 11px; padding: 13px; }
.record-row { display: flex; justify-content: space-between; gap: 14px; align-items: center; }
.record-row small,
.trace-card small { display: block; color: #8195a2; margin-top: 5px; }
.trace-heading { display: flex; justify-content: space-between; gap: 16px; align-items: flex-start; }
.trace-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px; margin: 12px 0 0; }
.trace-grid .wide { grid-column: 1/-1; }
.trace-grid dt { color: #8195a2; font-size: 11px; font-weight: 800; text-transform: uppercase; }
.trace-grid dd { margin: 6px 0 0; line-height: 1.6; }
.context-disclosure,
.proposal-review { margin-top: 12px; border: 1px solid #31505c; border-radius: 10px; padding: 12px; background: #08131a; }
.context-disclosure summary,
.proposal-review summary,
.danger-zone summary { cursor: pointer; font-weight: 900; color: #d8f2f2; }
.context-disclosure h3,
.proposal-review h3 { margin: 14px 0 8px; font-size: 13px; color: #9fb8c3; }
pre { overflow: auto; max-height: 420px; white-space: pre-wrap; overflow-wrap: anywhere; background: #050c11; border: 1px solid #20333d; border-radius: 9px; padding: 12px; color: #c9dce4; font: 12px/1.65 ui-monospace, SFMono-Regular, Menlo, monospace; }
.proposal-payload { max-height: 640px; border-color: #35636a; }
.review-section + .review-section { margin-top: 12px; }
.decision-rationale { display: grid; gap: 8px; margin-top: 14px; font-weight: 800; }
.decision-actions { display: flex; gap: 8px; margin-top: 10px; }
.row-actions { display: flex; align-items: center; gap: 10px; }
.row-actions a { color: #8de5e0; }
.muted { color: #95a7b3; line-height: 1.8; }
.diagnostic { color: #d9b896; border-inline-start: 2px solid #755b3c; padding-inline-start: 10px; }
.inline-form,
.form-grid { display: flex; gap: 10px; align-items: end; margin-top: 14px; }
.inline-form input[type='file'] { flex: 1; }
.form-grid { display: grid; grid-template-columns: 1fr 1fr; }
.form-grid label { display: grid; gap: 6px; color: #aebec8; }
.form-grid .full { grid-column: 1/-1; }
.form-grid button { justify-self: start; }
input,
textarea { width: 100%; box-sizing: border-box; background: #071119; border: 1px solid #334754; color: #e7f0f4; border-radius: 9px; padding: 10px; font: inherit; }
textarea { min-height: 88px; resize: vertical; }
.danger-button { border-color: #71434a; background: #2a161a; color: #ffc5c8; }
.danger-zone { border-color: #654d2e; background: #211a0f; }
.danger-body { margin-top: 14px; }
.mono { font-family: ui-monospace, SFMono-Regular, Menlo, monospace; }
.break-all { overflow-wrap: anywhere; word-break: break-all; }
.config-list { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 10px; }
.config-list dt { color: #879aa7; font-size: 12px; }
.config-list dd { margin: 6px 0 0; font-weight: 800; }
.danger-text { color: #ffb6b8; }
.empty { color: #788d99; text-align: center; padding: 22px; }
...
@media (max-width: 760px) {
  .system-workspace { padding: 12px; }
  .workspace-top { display: block; }
  .top-actions { margin-top: 16px; }
  .workspace-grid { grid-template-columns: 1fr; }
  .workspace-left { position: static; }
  .rail-link { display: inline-block; }
  .metric-strip,
  .metric-strip.compact,
  .status-grid,
  .config-list,
  .trace-grid { grid-template-columns: 1fr; }
  .trace-grid .wide { grid-column: auto; }
