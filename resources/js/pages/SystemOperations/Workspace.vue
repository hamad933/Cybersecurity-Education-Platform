    ...
    <div class="workspace-grid">
      ...
      <main class="workspace-center">
        ...
        <template v-else-if="surface === 'processing'">
          <section class="panel">
            <p class="section-kicker">Processing Runs</p>
            <h2>التنفيذ التقني الفعلي</h2>
            ...
            <div class="table-wrap">
              <table>
                ...
                <tbody>
                  <tr v-for="run in state.processing?.runs ?? []" :key="run.id">
                    <td>
                      <bdi dir="ltr">{{ run.type }}</bdi>
                    </td>
                    <td>
                      <span class="state-pill" dir="ltr">{{ run.status }}</span>
                    </td>
                    <td dir="ltr">{{ run.attempt_count }} / {{ run.max_attempts }}</td>
                    <td>{{ when(run.started_at) }}</td>
                    <td>{{ when(run.completed_at) }}</td>
                  </tr>
                  <tr v-if="(state.processing?.runs ?? []).length === 0">
                    <td colspan="5" class="empty">لا توجد Processing Runs مسجلة.</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <section class="panel">
            ...
            <div class="metric-strip compact">
              <div>
                <span>Pending</span><strong>{{ count(state.outbox?.counts, 'pending') }}</strong>
              </div>
              <div>
                <span>Leased</span><strong>{{ count(state.outbox?.counts, 'leased') }}</strong>
              </div>
              <div>
                <span>Dispatched</span
                ><strong>{{ count(state.outbox?.counts, 'dispatched') }}</strong>
              </div>
              <div>
                <span>Failed</span><strong>{{ count(state.outbox?.counts, 'failed') }}</strong>
              </div>
            </div>
          </section>
          <details class="deep-workspace">
            <summary>فتح مساحة الفحص العميق</summary>
            <div class="deep-grid">
              <article v-for="run in state.processing?.runs ?? []" :key="`deep-${run.id}`">
                <bdi dir="ltr">{{ run.id }}</bdi>
                <p>{{ run.safe_error_message || 'لا توجد رسالة خطأ آمنة.' }}</p>
                <small dir="ltr"
                  >worker={{ run.worker_identifier || '—' }} · category={{
                    run.error_category || '—'
                  }}</small
                >
              </article>
        ...
        <template v-else-if="surface === 'validation'">
          ...
          <section class="panel">
            ...
            <div class="metric-strip compact">
              <div>
                <span>Verified</span><strong>{{ count(packageCounts, 'verified') }}</strong>
              </div>
              <div>
                <span>Exported</span><strong>{{ count(packageCounts, 'exported') }}</strong>
              </div>
              <div>
                <span>Rejected</span><strong>{{ count(packageCounts, 'rejected') }}</strong>
              </div>
            </div>
            <div class="record-list">
              <article v-for="pkg in packagesAsRecords" :key="pkg.id" class="record-row">
                <div>
                  <strong
                    ><bdi dir="ltr">{{ pkg.package_type }}</bdi></strong
                  ><small
                    ><bdi dir="ltr">{{ pkg.owner_module }}</bdi> · {{ when(pkg.created_at) }}</small
                  >
                </div>
        ...
        <template v-else-if="surface === 'ai-bridge'">
          <section class="panel" id="manual-ai-export">
            <p class="section-kicker" dir="ltr">MANUAL_ONLY</p>
          ...
          <section class="panel">
            <h2>نتائج بانتظار قرار بشري</h2>
            <div class="record-list">
              <article v-for="result in state.results ?? []" :key="result.id" class="decision-row">
                <div>
                  <bdi dir="ltr" class="mono">{{ short(result.result_digest, 16) }}</bdi
                  ><small>{{ when(result.imported_at) }}</small>
                </div>
                <span class="state-pill" dir="ltr">{{ result.status }}</span
                ><template v-if="result.status === 'pending_review'">
                  <textarea
                    v-model="decisionRationales[result.id]"
                    maxlength="2000"
                    placeholder="مبرر القرار البشري"
                  />
                  <div class="decision-actions">
                    <button @click="decideAi(result.id, 'ACCEPT_AS_DRAFT')">قبول كمسودة</button
                    ><button class="danger-button" @click="decideAi(result.id, 'REJECT')">
                      رفض
                    </button>
                  </div></template
                >
        ...
        <template v-else-if="surface === 'backups'">
          ...
          <section class="panel">
            <h2>Restore Runs</h2>
            <div class="record-list">
              <article v-for="restore in state.restores ?? []" :key="restore.id" class="record-row">
                <div>
                  <strong
                    ><bdi dir="ltr">{{ restore.target_database }}</bdi></strong
                  ><small>{{ when(restore.started_at) }} → {{ when(restore.completed_at) }}</small>
                </div>
        ...
        <template v-else-if="surface === 'audit'">
          ...
          <section class="panel">
            ...
            <div class="table-wrap">
              <table>
                ...
                <tbody>
                  <tr v-for="record in state.records ?? []" :key="record.id">
                    <td dir="ltr">#{{ record.sequence_no }}</td>
                    <td>
                      <bdi dir="ltr">{{ record.action }}</bdi>
                    </td>
                    <td>
                      <bdi dir="ltr">{{ record.target_type }}</bdi>
                    </td>
                    <td>
                      <span class="state-pill" dir="ltr">{{ record.outcome }}</span>
                    </td>
                    <td>{{ when(record.occurred_at) }}</td>
                  </tr>
                  <tr v-if="(state.records ?? []).length === 0">
                    <td colspan="5" class="empty">لا توجد سجلات تدقيق.</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <details class="deep-workspace">
            <summary>فتح Hash / Correlation Inspector</summary>
            <div class="deep-grid">
              <article v-for="record in state.records ?? []" :key="`hash-${record.id}`">
                <bdi dir="ltr">correlation={{ record.correlation_id }}</bdi
                ><small dir="ltr">previous={{ record.previous_hash || 'GENESIS' }}</small
                ><small dir="ltr">record={{ record.record_hash }}</small>
              </article>
            </div>
          </details>
        </template>
              ...
              <article v-for="pkg in packagesAsRecords" :key="pkg.id" class="record-row">
                <div>
                  <strong
                    ><bdi dir="ltr">{{ pkg.package_type }}</bdi></strong
                  ><small
                    ><bdi dir="ltr">{{ short(pkg.package_digest, 16) }}</bdi> ·
                    {{ when(pkg.created_at) }}</small
                  >
                </div>
                <span class="state-pill" dir="ltr">{{ pkg.status }}</span>
              </article>
      ...
      <aside class="workspace-right" aria-label="السياق الفريد">
        <template v-if="surface === 'health'"
          ...
          <small>هذه المساحة تصف الأثر فقط؛ التفاصيل التقنية تبقى في المركز.</small></template
        >
        <template v-else-if="surface === 'processing'"
          ><p class="rail-label">اعتماديات</p>
          <h2>حدود المعالجة</h2>
          <p>هذه المساحة تملك ingestion / extraction / processing / staging التقني فقط.</p>
          <p>جودة المصدر والادعاء ليست قرارًا من Queue أو Worker.</p></template
        >
        <template v-else-if="surface === 'validation'"
          ><p class="rail-label">حد الملكية</p>
          <h2>تحقق تقني فقط</h2>
          <p>التحقق هنا يختبر البنية، النوع، provenance، hashes وحدود الحزمة.</p>
          <p>قرار جودة المعرفة يبقى لدى Research &amp; Quality.</p></template
        >
        <template v-else-if="surface === 'ai-bridge'"
          ...
          <p>لا API provider، لا polling، لا embeddings، ولا نشر تلقائي.</p>
        ...
        <template v-else-if="surface === 'audit'"
          ...
          <h2>Append-only</h2>
          <p>
            لا تعرض مساحة W05 أوامر update/delete للسجل. الفحص العميق يعرض hashes فقط.
          </p></template
        >
        <template v-else-if="surface === 'releases'"
          ...
          <h2>لا Deployment</h2>
          <p>
            المعروض هو package/release validation فقط. لا توجد صلاحية أو workflow للنشر الإنتاجي.
          </p></template
        >
        <template v-else-if="surface === 'configuration'"
...
.deep-grid article {
  display: grid;
  gap: 5px;
  border-inline-start: 2px solid #31545e;
