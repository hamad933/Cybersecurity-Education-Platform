...
describe("Simulation & Enterprise governed workspace", () => {
  ...
  it("renders enterprise topology in CENTER and unique object context in RIGHT without raw JSON", () => {
    ...
    expect(
      wrapper.get('[data-testid="enterprise-topology-canvas"]').text(),
    ).toContain("EDGE-01");
    expect(
      wrapper.get('[data-testid="enterprise-topology-canvas"]').text(),
    ).toContain("APP-01");
    expect(
      wrapper.get('[data-testid="enterprise-object-context"]').text(),
    ).toContain("twin-digest");
  ...
  it("renders scenario phases once as orchestration flow and keeps validation in RIGHT", () => {
    ...
    expect(wrapper.get('[data-testid="scenario-properties"]').text()).toContain(
      "trace_required",
    );
  ...
  it("renders lab steps as task graph and properties in the right inspector", () => {
    ...
    expect(wrapper.get('[data-testid="lab-task-graph"]').text()).toContain(
      "observe",
    );
    expect(wrapper.get('[data-testid="lab-task-graph"]').text()).toContain(
      "correlate",
    );
    expect(wrapper.get('[data-testid="lab-properties"]').text()).toContain(
      "requires_trace",
    );
  });
  ...
  it("keeps Operations as a mode inside the active Run and RIGHT as interpretation only", async () => {
    ...
    expect(wrapper.get('[data-testid="run-interpretation"]').text()).toContain(
      "التنفيذ الداخلي نشط",
    );
    expect(
      wrapper.get('[data-testid="simulation-primary-nav"]').text(),
    ).not.toContain("Operations");

    await wrapper.get('[data-testid="run-operations-mode"]').trigger("click");
    expect(
      wrapper.get('[data-testid="run-operations-panel"]').text(),
    ).toContain("حفظ Snapshot");
  });
  ...
  it("keeps Replay, AAR, and Compare inside Results and does not invent comparison data", async () => {
    ...
    expect(wrapper.get('[data-testid="result-analysis"]').text()).toContain(
      "دون استنتاج Mastery",
    );
