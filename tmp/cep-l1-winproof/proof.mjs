...
async function awaitReady(id,ms=30000){return until(()=>{const s=manager.session(id);if(s&&['completed','error'].includes(s.state))throw new Error('READY_ABORTED:'+JSON.stringify(s));return s?.controlPipe&&s?.epoch?s:null},ms,()=>({session:manager.session(id),events:events.slice(-20)}))}
