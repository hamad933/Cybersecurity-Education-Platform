...
async function awaitExit(id,ms=30000){return until(()=>{const s=manager.session(id);return s && ['completed','error'].includes(s.state)?s:null},ms,()=>({session:manager.session(id),output:text(id).slice(-5000),events:events.slice(-20)}))}
...
async function powershellProof(){
 ...
 manager.write(sessionId,Buffer.from("Write-Output 'CEP_PS_OK'; Write-Output $env:CEP_PROOF_ENV; Write-Output (Get-Location).Path; exit 7\r",'utf8'));
 ...
 const epoch1=ready.epoch; await manager.restart(sessionId); const ready2=await awaitReady(sessionId); manager.write(sessionId,Buffer.from("Write-Output 'CEP_RESTART_OK'; exit 0\r",'utf8')); const done2=await awaitExit(sessionId); const out2=text(sessionId);record('TERM-RESTART-STABLE-ID',ready2.id===sessionId&&ready2.restartCount===1&&ready2.epoch!==epoch1&&out2.includes('CEP_RESTART_OK')&&done2.exitCode===0,{sessionId:ready2.id,restartCount:ready2.restartCount,oldEpoch:epoch1,newEpoch:ready2.epoch});
}
async function cmdProof(){const id='cep-proof-cmd';manager.open({sessionId:id,label:'cmd.exe',executable:cmd,args:['/Q'],cols:90,rows:25});await awaitReady(id);manager.write(id,Buffer.from('echo CEP_CMD_OK & exit /b 5\r','utf8'));const done=await awaitExit(id);const out=text(id);record('TERM-CMD-PROFILE',out.includes('CEP_CMD_OK')&&done.exitCode===5,{exitCode:done.exitCode,output:out.slice(-2000)});}
