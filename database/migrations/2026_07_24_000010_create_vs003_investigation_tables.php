...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('vs003_investigation_cases', function (Blueprint $table): void {
            ...
            $table->uuid('dataset_revision_id');
            $table->string('case_id', 100);
            $table->string('expected_outcome', 32);
            $table->jsonb('definition');
            $table->char('digest', 64);
            $table->timestampsTz();
            $table->unique(['scenario_revision_id', 'case_id']);
            $table->foreign('scenario_revision_id')->references('id')->on('scenario_revisions')->restrictOnDelete();
            $table->foreign('dataset_revision_id')->references('id')->on('vs003_telemetry_dataset_revisions')->restrictOnDelete();
        });
        ...
        Schema::create('vs003_triage_records', function (Blueprint $table): void {
            ...
            $table->jsonb('alternative_hypotheses');
            $table->jsonb('missing_data');
        ...
        Schema::create('vs003_custody_events', function (Blueprint $table): void {
            ...
            $table->jsonb('source_event_ids');
            $table->char('source_digest', 64);
            $table->timestampTz('collected_at');
            $table->string('storage_reference', 160);
            $table->string('copy_kind', 24);
        ...
        Schema::create('vs003_containment_proposals', function (Blueprint $table): void {
            ...
            $table->text('expected_effect');
            $table->text('risk');
            $table->text('rollback_condition');
            ...
            $table->foreign('scenario_run_id')->references('id')->on('scenario_runs')->restrictOnDelete();
            $table->foreign('actor_id')->references('id')->on('owner_accounts')->restrictOnDelete();
            $table->foreign('approved_by')->references('id')->on('owner_accounts')->restrictOnDelete();
        });
        Schema::create('vs003_control_revisions', function (Blueprint $table): void {
            $table->uuid('id')->primary();
            $table->string('control_id', 100);
            $table->unsignedInteger('revision');
            $table->string('state', 24);
            $table->jsonb('definition');
            $table->char('digest', 64);
            $table->uuid('remediates_run_id');
            $table->timestampTz('published_at');
            $table->timestampsTz();
            $table->unique(['control_id', 'revision']);
        ...
        Schema::create('vs003_verification_replays', function (Blueprint $table): void {
            ...
            $table->uuid('control_revision_id');
            $table->char('original_timeline_digest', 64);
            $table->char('verification_timeline_digest', 64);
            ...
            $table->foreign('control_revision_id')->references('id')->on('vs003_control_revisions')->restrictOnDelete();
        });
    ...
    public function down(): void
    {
        ...
        Schema::dropIfExists('vs003_verification_replays');
        Schema::dropIfExists('vs003_control_revisions');
        Schema::dropIfExists('vs003_containment_proposals');
        Schema::dropIfExists('vs003_custody_events');
        Schema::dropIfExists('vs003_triage_records');
        Schema::dropIfExists('vs003_investigation_alerts');
        Schema::dropIfExists('vs003_investigation_cases');
        Schema::dropIfExists('vs003_telemetry_dataset_revisions');
    }
};
