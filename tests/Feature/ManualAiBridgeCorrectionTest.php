...
final class ManualAiBridgeCorrectionTest extends TestCase
{
    ...
    public function test_human_decision_edit_into_new_draft_creates_draft_lesson_and_updates_status(): void
    {
        ...
        $bridge->decide(
            ...
            $actorId,
            'edit_into_new_draft',
            'Approved by curriculum team after verification against baseline standard.'
        );
    ...
    public function test_multi_proposal_decisions_state_machine_and_append_only_history(): void
    {
        ...
        $resultPayload = [
            ...
            'knowledge_unit_id' => 'KU-SEC-10',
            'proposed_blocks' => [
                ['proposal_id' => 'prop_1', 'type' => 'paragraph', 'body' => 'محتوى تجريبي صالح يقبله النظام.'],
                ['proposal_id' => 'prop_2', 'type' => 'paragraph', 'body' => 'محتوى تجريبي صالح يقبله النظام أيضا.']
            ],
            'citation_claim_ids' => ['WIN-AUTH-002'],
            'derived_from_revision_id' => null,
            'authority_baseline_id' => config('vs001.authority_baseline_id'),
        ...
        $bridge->decide($imported->id, 'prop_1', $actorId, 'defer', 'Need more time.');
        $imported->refresh();
        ...
        // Note: For "prop_1" mock payload body "مقترح الأول" does not match constraints (min length or allowed characters)
        // Update proposed block body for valid drafts where we simulate "accept" or "edit_into_new_draft"
        // We bypass the HTTP layer so direct draft creation fails inside AiDraftCreationService due to validation unless body is acceptable
        $bridge->decide($imported->id, 'prop_1', $actorId, 'accept', 'Approved 1.');
        $imported->refresh();
        ...
        $failed = false;
        try {
            $bridge->decide($imported->id, 'prop_1', $actorId, 'reject', 'Wait nevermind.');
        ...
        $bridge->decide($imported->id, 'prop_2', $actorId, 'request_evidence', 'Need evidence.');
        $imported->refresh();
        ...
        $bridge->decide($imported->id, 'prop_2', $actorId, 'reject', 'Rejecting it.');
        $imported->refresh();
    ...
    public function test_all_terminal_reject_rolls_up_to_rejected(): void
    {
        ...
        $bridge->decide($imported->id, 'prop_1', $actorId, 'reject', 'Rejected 1.');
    ...
    public function test_import_result_rejects_if_actor_does_not_own_prompt_package(): void
    {
        ...
        $packagesBefore = \App\Modules\Platform\Packages\PortablePackageRecord::query()->count();
        ...
        $failed = false;
        try {
            // Act as Owner B directly to service
            $bridge->importResult($fp, $actorIdB);
        ...
        $this->assertTrue($failed, 'Import should fail due to actor mismatch.');
        ...
        $this->assertSame($packagesBefore, \App\Modules\Platform\Packages\PortablePackageRecord::query()->count());
    ...
    public function test_exact_retry_idempotency_after_overall_terminal_closure(): void
    {
        ...
        // 2. Decide terminal accept -> closes result via direct service call to bypass stale shared adapter
        $bridge->decide($imported->id, 'prop_1', $actorId, 'accept', 'Approved 1.');
        ...
        // 4. Mismatched Retry MUST throw LogicException from the direct service
        $failed = false;
        try {
            $bridge->decide($imported->id, 'prop_1', $actorId, 'accept', 'Different rationale now.');
        } catch (\LogicException $e) {
            $failed = true;
        }
        $this->assertTrue($failed, 'Different rationale on terminal result should fail closed.');
        ...
        $bridge->decide($imported2->id, 'prop_A', $actorId, 'reject', 'Rej A');
        $bridge->decide($imported2->id, 'prop_B', $actorId, 'reject', 'Rej B');
        ...
        $failed2 = false;
        try {
            $bridge->decide($imported2->id, 'prop_A', $actorId, 'accept', 'Changed my mind');
        } catch (\LogicException $e) {
            $failed2 = true;
        }
        $this->assertTrue($failed2, 'Mismatch decision on terminal rejected result should fail closed.');
    }
    ...
    public function test_import_result_rejects_if_ai_claims_mismatch_verified_scope(): void
    {
        ...
        $failed = false;
        try {
            $bridge->importResult($fp, $actorId);
        } catch (\InvalidArgumentException $e) {
            $failed = true;
        }
        ...
        $this->assertTrue($failed, 'Import should fail due to mismatched provenance scope claims.');
    }
}