...
class ProcessingRunTest extends TestCase
{
    ...
    public function test_migration_down_preflight_verifies_duplicates(): void
    {
        ...
        // 5b. Verify max_attempts and worker_identifier are retained (owned by older migration)
        $this->assertTrue(\Illuminate\Support\Facades\Schema::hasColumn('processing_runs', 'worker_identifier'));
        $this->assertTrue(\Illuminate\Support\Facades\Schema::hasColumn('processing_runs', 'max_attempts'));
        
        // 6. Older migration down verification (proving reversibility without max_attempts conflict)
        $olderMigration = require database_path('migrations/2026_07_25_000012_create_v1_integration_release_tables.php');
        $olderMigration->down();
        
        $this->assertFalse(\Illuminate\Support\Facades\Schema::hasColumn('processing_runs', 'max_attempts'));
        
        // 6b. Older migration up
        $olderMigration->up();
        
        // 7. Succeed up
        $migration->up();
        
        // 8. Verify schema (up succeeded)
        $this->assertTrue(\Illuminate\Support\Facades\Schema::hasColumn('processing_runs', 'retry_ordinal'));
        $this->assertTrue(\Illuminate\Support\Facades\Schema::hasColumn('processing_runs', 'worker_identifier'));
    ...
    public function test_it_supports_worker_liveness_heartbeats(): void
    {
        ...
        // Future anomaly
        $worker->last_seen_at = now()->addSeconds(200);
        $this->assertEquals('UNKNOWN', $worker->status());
    }

    public function test_queue_worker_execution_hook_refreshes_heartbeat(): void
    {
        // Bind the new subscriber to the dispatcher to prove the execution hook
        $dispatcher = $this->app->make(\Illuminate\Events\Dispatcher::class);
        $subscriber = $this->app->make(\App\Modules\Platform\Processing\QueueWorkerHeartbeatSubscriber::class);
        ...
        $dispatcher->dispatch(new \Illuminate\Queue\Events\Looping('test-provider', 'default'));
        
        $heartbeat = WorkerHeartbeat::query()->where('worker_identifier', $workerId)->first();
        $this->assertNotNull($heartbeat);
        $this->assertEquals('test-provider', $heartbeat->provider);
        $this->assertEquals($workerId, $heartbeat->worker_identifier);
        $this->assertEquals('HEALTHY', $heartbeat->status());
        
        // Prove aged > TTL => DOWN
        $heartbeat->last_seen_at = now()->subSeconds(600);
        $heartbeat->save();
        
        $this->assertEquals('DOWN', $heartbeat->status());
        
        // Fire real JobProcessing event to refresh
        $jobMock = $this->createMock(\Illuminate\Contracts\Queue\Job::class);
        $dispatcher->dispatch(new \Illuminate\Queue\Events\JobProcessing('test-provider', $jobMock));
        
        $heartbeat->refresh();
        $this->assertEquals('HEALTHY', $heartbeat->status());
        $this->assertTrue($heartbeat->last_seen_at->gt(now()->subSeconds(5)));
    }
